...
function policy:ban_at_cloudflare(ip_address)
    ...
    local res, err = httpc:request_uri(string.format("https://api.cloudflare.com/client/v4/zones/%s/firewall/access_rules/rules", self.config.cloudflare.zone_id), {
        method = 'POST',
        body = body,
        headers = {
    ...
    if not res then
        ngx.log(ngx.ERR, string.format("Failed to ban (for %s) at CloudFlare", ip_address), err)
        return false
    end
    ...
    ngx.log(ngx.NOTICE, string.format("Successfully banned %s (identifier %s) at CloudFlare", ip_address, response.result.id))
...
function policy:unban_at_cloudflare(identifier)
    ...
    local httpc = http.new()
    local res, err = httpc:request_uri(string.format("https://api.cloudflare.com/client/v4/zones/%s/firewall/access_rules/rules/%s", self.config.cloudflare.zone_id, identifier), {
        method = 'DELETE',
        body = body,
        headers = {
            ['X-Auth-Email'] = self.config.cloudflare.email,
    ...
    if not res then
        ngx.log(ngx.ERR, string.format("Failed to unban (for identifier %s) at CloudFlare", identifier), err)
        return false
    end
